...
dd if=bsi.exe of=cpprest_2_10.dll bs=1 skip=$(($start_bytes_cpprest)) count=$(($end_bytes_cpprest - $start_bytes_cpprest)) status=progress
...
dd if=bsi.exe of=VC_redist.x64.exe bs=1 skip=$(($start_bytes_vcredist)) count=$(($end_bytes_vcredist - $start_bytes_vcredist)) status=progress
